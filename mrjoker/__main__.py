...
@run_async
def Villain_about_callback(update, context):
    query = update.callback_query
    if query.data == "aboutmanu_":
        ...
        query.message.edit_text(
            ...
            f"\n\nYou Can get support {dispatcher.bot.first_name} by joining [DEV CHANNEL](https://t.me/lkhitech).\n"
            f"",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
        ...
        query.message.edit_text(
            text=f"*{dispatcher.bot.first_name} Is the redisigned version Maria  for the best performance.*"
            f"\n\nBased on  [Mr.Joker](https://t.me/lkhitech)."
            f"\n\n{dispatcher.bot.first_name}'s source code was written by LHIECH™️ and DEVELOPERS"
        ...
        query.message.edit_text(
            ...
            f"\n🔹 Always try to keep credits, so \n  This hardwork is done by @lkhitech team  spending many sleepless nights.. So, Respect it."
            f"\n🔹 Some modules in this bot is owned by different authors, So, \n  All credits goes to only LKHITECH team"
            f"\n🔹 If you need to ask anything about \n  this bot, Go @{SUPPORT_CHAT}."
            f"\n🔹 If you asking nonsense in Support \n  Chat, you will get warned/banned."
            f"\n🔹 All api's we used owned by originnal authors \n  Some api's we use Free version \n  Please don't overuse AI Chat."
            f"\n🔹 We don't Provide any support to forks,\n  So these terms and conditions not applied to forks \n  If you are using a fork of Villain bot we are not resposible for anything."
...
def get_help(update, context):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        update.effective_message.reply_text(
            "Contact me in PM to get the list of possible commands come baby.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        Villain_about_callback, pattern=r"aboutmanu_"
    )
